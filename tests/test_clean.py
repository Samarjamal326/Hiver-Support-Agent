...
def test_english_heuristic_not_fooled_by_numeric_mentions():
    """Regression test: English tweets padded with 2+ numeric @mentions must not be
    misclassified as non-English after the strip_all_mentions fix.

    Bug: before the fix, short English text like 'India needs @115888' had its
    ASCII-letter ratio diluted by the numeric handle characters, causing it to fall
    below the 0.6 threshold and be incorrectly discarded.
    """
    # Clearly English sentences padded with numeric @handles
    cases_should_pass = [
        "India needs @115888",                        # previously dropped at ratio 0.526
        "It really sucks that @551 and @3243 aren't on @115888.",  # ratio 0.532 before fix
        "I'm very confused @33406 @115888 @SpotifyCares",          # ratio 0.565 before fix
        "Can you add this? @115888 @SpotifyCares @22875",
    ]
    for text in cases_should_pass:
        assert is_english_heuristic(text, threshold=0.6) is True, (
            f"Expected English (True) for: {text!r}  "
            f"(strip_all_mentions gives: {strip_all_mentions(text)!r})"
        )

    # Genuinely non-English strings must still be rejected
    cases_should_fail = [
        "Привет, помогите пожалуйста восстановить мой аккаунт",
        "音楽が再生されません",
        "مرحبا، أحتاج إلى مساعدة في حسابي",
    ]
    for text in cases_should_fail:
        assert is_english_heuristic(text, threshold=0.6) is False, (
            f"Expected non-English (False) for: {text!r}"
        )

    # Via clean_dataframe: numeric-mention-padded English rows must survive
    df = pd.DataFrame(
        [
            {"text": "India needs @115888", "expected": "keep"},
            {"text": "Привет, помогите пожалуйста", "expected": "drop"},
            {"text": "I'm confused @33406 @115888 @SpotifyCares", "expected": "keep"},
        ]
    )
    cleaned = clean_dataframe(df, english_threshold=0.6, verbose=False)
    # text_clean should preserve the mid-sentence/trailing @handles (not stripped)
    kept_texts = cleaned["text_raw"].tolist()
    assert "India needs @115888" in kept_texts
    assert "I'm confused @33406 @115888 @SpotifyCares" in kept_texts
    assert not any("Привет" in t for t in kept_texts)
    # Confirm text_clean still contains the @handles (mentions not removed from output)
    for _, row in cleaned.iterrows():
        assert "@" in row["text_clean"] or "India" in row["text_clean"]
